...
- **任务管理**：创建、编辑和删除任务，设置截止日期和优先级
- **多标签支持**：为任务添加多个标签，灵活分类管理
- **标签管理**：创建、编辑和删除标签
- **任务状态追踪**：标记任务完成状态，直观显示进度
- **简洁界面**：清晰直观的用户界面，操作简单，支持文本居中和列宽自适应等UI优化
- **智能标签提取**：从任务标题中自动提取标签（使用 #标签 格式）
- **多标签筛选**：通过多个标签组合筛选任务
- **异步加载**：任务列表异步加载，提升界面流畅度
...
- **日期选择器**：自定义日期选择，支持“无截止日期”状态，防止选择过去日期
- **多标签体系**：任务可灵活添加/移除多个标签，支持批量标签筛选
- **智能输入**：输入任务时自动识别 #标签
- **高效交互**：任务编辑、删除、完成状态切换一键直达，表格内容居中显示，列宽智能适应
- **MVC分层架构**：代码清晰易维护，便于扩展
- **优先级管理**：支持多级优先级，界面高亮显示
- **异步数据加载**：任务列表等数据采用后台异步加载，避免UI卡顿，提升用户体验

## 技术实现细节

### 模型层

- **SQLAlchemy ORM**：对象关系映射，简化数据库操作
- **多对多关系**：任务与标签多对多，结构灵活
- **数据验证**：基础数据校验

### 控制器层

- **业务逻辑封装**：所有业务逻辑集中于控制器，视图层仅关注UI
- **数据转换**：模型与视图数据双向转换
- **智能标签提取**：支持从任务标题自动识别标签

### 视图层

- **PySide6 UI**：基于Qt框架的现代化用户界面
- **自定义控件**：如日期选择器、优先级高亮、标签选择等
- **界面美观简洁，交互友好**
- **异步任务处理**：使用 `QThreadPool` 和 `QRunnable` 实现数据库操作的异步执行，防止UI阻塞，提升用户体验。
...
- [x] MVC架构重构
- [x] 任务优先级设置
- [x] UI 优化 (文本居中, 列宽自适应, 复选框居中)
- [x] 异步加载任务列表
- [x] 任务筛选和搜索
- [ ] 数据导入导出
- [ ] 任务提醒功能
- [x] 将其他数据库操作（增删改任务/标签）改为异步
- [ ] 数据统计和可视化
- [ ] 用户账户系统
- [ ] 云同步功能
- [ ] 移动端应用
...
本项目采用本人亲自许可，仰头默念 **"Amazing Boom 请赐予我许可吧！"** 即可获取许可。
