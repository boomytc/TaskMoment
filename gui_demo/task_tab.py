...
class EditTaskDialog(QDialog):
    def __init__(self, session, task: Task | None = None, parent=None):
        ...
        if task:
            ...
            if task.due_date: # <-- Only set if task has a due date
                ...
                self.date_edit.setDate(QDate()) # <-- Clear date
            if task.tag:
    ...
    def get_data(self):
        ...
        # If tag in title, use that instead
        if tag_in_title:
...
class TaskTab(QWidget):
    ...
    def _refresh_tag_combo(self):
        current_data = self.new_tag_combo.currentData()
        self.new_tag_combo.clear()
        self.new_tag_combo.addItem("无标签", None)
        for tag in self.session.query(Tag).order_by(Tag.tag).all():
            self.new_tag_combo.addItem(tag.tag, tag.id)
        if current_data:
            index = self.new_tag_combo.findData(current_data)
            if index >= 0:
                self.new_tag_combo.setCurrentIndex(index)
